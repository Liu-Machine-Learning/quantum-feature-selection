%% Mutual-information QUBO feature selection for plasma dataset
%
% Dataset format:
...
% - Discretize each standardized feature into 20 quantile bins
%
% Mutual-information QUBO:
%
...
% Match StandardScaler behavior for constant variables.
...
if any(~isfinite(Xz), "all")
    error("The standardized matrix contains nonfinite values.");
end
...
% )
%
% quantileBinLikeNumPy is defined at the end of this script.
%% ------------------------------------------------------------
...
% mutual_info_score. Therefore, MI is measured in nats.
%% ------------------------------------------------------------
...
%   Q_ij = (1-alpha) * R_ij
%
% The Python reference does not rescale I or R.
%% ------------------------------------------------------------
...
% direct coefficient comparison with the Python implementation.
%% ------------------------------------------------------------
...
for i = 1:n_features
    ...
    for j = i+1:n_features
        ...
        % Iskay stores each pair once, so the dictionary
...
%   x'Qx
%
% The complete symmetric Q matrix is supplied directly.
%% ------------------------------------------------------------
...
%
% Only the solve call is timed. Dataset loading, z-scoring,
% discretization, MI calculation, QUBO construction, and
...
% Independently verify the returned objective value
...
%   )
    ...
    % np.unique sorts and removes repeated quantile edges.
    edges = unique(edges);
    ...
    % For np.digitize(..., right=True), equality remains in the
    % lower bin. The bin index is therefore the number of
    % interior edges strictly smaller than the observation.
