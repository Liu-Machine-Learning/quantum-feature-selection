%% Fisher-score QUBO feature selection for Shotgun dataset
%
% Dataset format:
...
% - Encode ASD = 1 and TD = 0
% - Keep the first n_keep valid features
% - Transpose into subjects-by-features format
% - Standardize using population standard deviation
%
% Fisher QUBO:
%
...
% Remove feature rows containing any missing values
%
% This is performed before subject filtering, matching Python:
%
% valid_feature_mask =
...
%% ------------------------------------------------------------
% Keep first n_keep valid features
%
% Python:
...
% StandardScaler effectively uses a scale of 1 for constant features.
...
%% ------------------------------------------------------------
% Pairwise covariance on standardized data
%
% Python:
% C = np.cov(Xz, rowvar=False)
%
% Both NumPy and MATLAB use sample covariance here.
%% ------------------------------------------------------------

C = cov(Xz);

% Absolute covariance represents feature redundancy
C = abs(C);
...
%% ------------------------------------------------------------
% Scale Fisher scores and covariance
%% ------------------------------------------------------------
...
%
% Diagonal:
%   Q_ii = -alpha * F_scaled_i
%
% Off-diagonal:
%   Q_ij = (1-alpha) * C_scaled_ij
%% ------------------------------------------------------------
...
%
% MATLAB does not require this dictionary. It is printed so the
% coefficients can be compared directly with the Python output.
%% ------------------------------------------------------------
...
for i = 1:n_features
    ...
    for j = i+1:n_features
        ...
        % Iskay stores each quadratic pair once, so its dictionary
...
%   x'Qx
%
% The complete symmetric matrix is passed directly. Do not
% multiply the off-diagonal matrix entries by two again.
%% ------------------------------------------------------------
...
%
% Only the solve call is timed. Loading, preprocessing, score
% calculation, QUBO construction, and printing are excluded.
%% ------------------------------------------------------------

fprintf("\nStarting MATLAB tabu-search Fisher-QUBO solver...\n");
...
fprintf("MATLAB Fisher-QUBO solve runtime: %.6f seconds\n", ...
